...
class SQLType:
    python: ClassVar[type[Any] | None] = None
...
class Integer(SQLType):
    ...
    def __init__(self, *, big: bool = False, small: bool = False, auto_increment: bool = False):
        ...
        if big and small:
            raise SchemaError('Integer column cannot be both big and small.')
...
class String(SQLType):
    ...
    def __init__(self, *, length: int | None = None, fixed: bool = False):
        self.length = length
        self.fixed = fixed
...
class ForeignKey(SQLType):
    def __init__(
        ...
        column: str,
        *,
        sql_type: SQLType | type[SQLType] | None = None,
        on_delete: str = 'CASCADE',
        on_update: str = 'NO ACTION',
    ):
        if not table or not isinstance(table, str):
            raise SchemaError('Missing table to reference (must be a string).')
        ...
        if on_delete not in valid_actions:
            raise SchemaError('on_delete must be one of %s.' % valid_actions)

        if on_update not in valid_actions:
            raise SchemaError('on_update must be one of %s.' % valid_actions)
...
class Array(SQLType):
    ...
    def __init__(self, sql_type: type[SQLType] | SQLType):
        if inspect.isclass(sql_type):
            sql_type = sql_type()
...
class Column:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(column_type, SQLType):
            raise TypeError('Cannot have non-SQLType derived column_type')
        ...
        self.index_name: str | None = None  # to be filled later
    ...
    def _qualifiers_dict(self) -> dict[str, bool]:
        return {attr: getattr(self, attr) for attr in ('nullable', 'default')}

    def _is_rename(self, other: Column) -> bool:
...
class SchemaDiff:
    ...
    def __init__(self, table: Table | type[Table], upgrade: dict[str, Any], downgrade: dict[str, Any]):
        self.table = table
    ...
    def to_sql(self, *, downgrade: bool = False) -> str:
        statements = []
...
class MaybeAcquire:
    def __init__(self, connection: asyncpg.Connection | None, *, pool: asyncpg.Pool):
...
class Table(metaclass=TableMeta):  # type: ignore # Pyright bug I think
    ...
    @classmethod
    async def create_pool(cls, uri: str, **kwargs: Any) -> asyncpg.Pool:
    ...
    @classmethod
    def acquire_connection(cls, connection: asyncpg.Connection) -> MaybeAcquire:
        return MaybeAcquire(connection, pool=cls._pool)

    @classmethod
    def write_migration(cls, *, directory: Path | str = 'migrations') -> bool | None:
    ...
    @classmethod
    async def drop(
        cls, *, directory: Path | str = 'migrations', verbose: bool = False, connection: asyncpg.Connection | None = None
    ) -> None:
    ...
    @classmethod
    async def insert(cls, connection: asyncpg.Connection | None = None, **kwargs) -> None:
        ...
        for column in cls.columns:
            try:
                value = kwargs[column.name]  # type: ignore # str | None is okay
            except KeyError:
                continue
    ...
    @classmethod
    def all_tables(cls: type[Self]) -> list[type[Self]]:
        return cls.__subclasses__()

    def diff(self, before: Table) -> SchemaDiff:
...
async def _table_creator(tables: Iterable[Table], *, verbose: bool = True) -> None:
    for table in tables:
        try:
...
def create_tables(*tables: Table, verbose: bool = True, loop: asyncio.AbstractEventLoop | None = None) -> None:
    if loop is None:
