...
class Stars(commands.Cog):
    ...
    def records_to_value(self, records: list[Any], fmt: Callable[[str], str], default: str = 'None!') -> str:
        if not records:
            return default

        emoji_lookup = lambda i: '\N{SPORTS MEDAL}' if i >= 3 else chr(0x1F947 + i)
    ...
    async def star_guild_stats(self, ctx: StarboardContext):
        ...
        e.add_field(
            name='Top Star Givers', value=self.records_to_value(top_givers, to_mention, default='No one!'), inline=False
        )
    ...
    @star.command(hidden=True, with_app_command=False, name='update_givers')
    @commands.is_owner()
