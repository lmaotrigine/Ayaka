...
LOGGING_CHANNEL = 956838275438489651
...
class LoggingHandler(logging.Handler):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        valid_record_name = record.name == 'discord.gateway' or (
            record.name != 'cogs.stats' and record.name.startswith('cogs.')
        )
...
class Stats(commands.Cog):
    ...
    def webhook(self) -> discord.Webhook:
        url = self.bot.config.stat_webhook
        hook = discord.Webhook.from_url(url, session=self.bot.session)
        return hook
    ...
    async def stats(self, ctx: GuildContext, *, member: discord.Member | None = None):
    ...
    @commands.is_owner()
    async def command_history_for(self, ctx: Context, days: Annotated[int, int | None] = 7, *, command: str):
    ...
    @commands.is_owner()
    async def command_history_cog(self, ctx: Context, days: Annotated[int, int | None] = 7, *, cog_name: str | None = None):
