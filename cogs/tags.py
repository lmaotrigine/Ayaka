...
class TagMakeModal(discord.ui.Modal, title='Create New Tag'):
    ...
    async def on_submit(self, interaction: discord.Interaction[Ayaka]) -> None:
        ...
        converter = TagName()
        try:
            name = await converter.convert(self.ctx, name)
...
class Tags(commands.Cog):
    ...
    async def edit(
        ...
        *,
        ...
        if len(content) > 2000:
            await ctx.send('Tag content can only be up to 2000 characters')
            return
    ...
    async def _list(
        ...
        ctx: GuildContext,
        *,
        member: discord.User = commands.param(default=lambda ctx: ctx.author, displayed_default='you'),
    ):
