...
class CannotEmableGatekeeper(Exception):
    pass
...
class Gatekeeper:
    ...
    Currently, this is implemented as a random captcha in a channel where a user must solve a random question
    through a modal.
    ...
    - ModConfig.automod_flags.gatekeeper
        ...
        This is different from whether gatekeeper is *actually* enabled.
    ...
    - started_at is None
        ...
        There is still a special case where this is true, but there
    ...
    def __repr__(self) -> str:
        ...
        joined = ' '.join(f'{name}={attr!r}' for name, attr in attrs)
...
class GatekeeperMessageModal(discord.ui.Modal, title='Started Message'):
...
class GatekeeperRateLimitConfirmationView(discord.ui.View):
    def __init__(self, *, existing_rate: tuple[int, int], author_id: int) -> None:
        ...
        self.existing_rate = existing_rate
...
class GatekeeperChannelSelect(discord.ui.ChannelSelect['GatekeeperSetupView']):
    ...
    async def callback(self, interaction: discord.Interaction[Ayaka]) -> None:
...
class GatekeeperSetupView(discord.ui.View):
    ...
    def __init__(self, cog: Mod, user: discord.abc.User, config: ModConfig, gatekeeper: Gatekeeper) -> None:
        ...
        self.selected_role = gatekeeper.role
        self.selected_message_id = gatekeeper.message_id
        ...
        self.setup_bypass_action.options = [
            ...
            discord.SelectOption(
                label='Ban User', value='ban', emoji='\N{HAMMER}', description='Ban the member if they talk before verifying'
            ),
        ]
    ...
    async def setup_message(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if self.gatekeeper.message is not None:
            await interaction.response.send_message(
                'Somehow you managed to press this while a message is already set up.', ephemeral=True
            )
            return
        if channel is None:
            await interaction.response.send_message(
        ...
        embed.set_footer(
            text='\u26a0\ufe0f This message was set up by the moderators of this server. This bot will never ask for your personal information, not is it related to Discord.'
        )
...
class GatekeeperVerifyButton(discord.ui.DynamicItem[discord.ui.Button], template='gatekeeper:verify:captcha'):
    ...
    async def from_custom_id(
        cls, interaction: discord.Interaction[Ayaka], item: discord.ui.Button, match: re.Match[str], /
    ) -> Self:
    ...
    async def interaction_check(self, interaction: discord.Interaction[Ayaka], /) -> bool:
...
class GatekeeperAlertMassbanButton(discord.ui.DynamicItem[discord.ui.Button], template='gatekeeper:alert:massban'):
    ...
    async def callback(self, interaction: discord.Interaction[Ayaka]) -> None:
        ...
        assert interaction.guild is not None
        assert interaction.message is not None
        members = self.cog._spam_check[interaction.guild.id].flagged_users
        if not members:
...
class TaggedRateLimit(Generic[V, HashableT]):
    def __init__(
        ...
        *,
        ...
        self.lookup: LRU[V, tuple[datetime.datetime, set[HashableT]]] = LRU(maxsize)
        self.rate = rate
        self.per = per
        self.key = key
        self.tagger = tagger
...
class SpamChecker:
    ...
    2) It checks if the content has been spammed 5 times in 15 seconds.
    3) It checks if new users have spammed 30 times in 35 seconds.
    4) It checks if "fast joiners" have spammed 5 times in 15 seconds.
    ...
    def check_gatekeeper(self, member: discord.Member, gatekeeper: Gatekeeper) -> list[discord.Member]:
        ...
        if rate != self._join_rate:
            # might be worth considering swapping over that tat/member list? probably complicated though
...
class Mod(commands.Cog):
    ...
    async def bot_check(self, ctx: Context) -> bool:
        if ctx.guild is None:
            return True
        # just so we don't get locked out of the bot due to bugs
    ...
    async def robomod_disable(
        ...
        if protection == 'all':
            updates = 'automod_flags = 0, mention_count = 0, broadcast_channel = NULL, alert_channel_id = NULL'
            ...
            updates = (
                f'automod_flags = guild_mod_config.automod_flags & ~{AutoModFlags.alerts.flag}, alert_channel_id = NULL'
            )
    ...
    async def massban(self, ctx: GuildContext, *, args: MassbanFlags):
        ...
        if args.channel:
            ...
            if args.ends:
                predicates_.append(lambda m: m.content.endswith(args.ends))  # type: ignore
            if args.match:
                try:
                    ...
                    predicates_.append(lambda m, x=_match: x.match(m.content))  # type: ignore
            if args.embeds:
                predicates_.append(args.embeds)  # type: ignore
            if args.files:
                predicates_.append(args.files)  # type: ignore
        ...
        if args.username:
            try:
                ...
                predicates.append(lambda m, x=_regex: x.match(m.name))  # type: ignore
