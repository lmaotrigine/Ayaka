...
class ModConfig:
    ...
    @classmethod
    async def from_record(cls, record: Any, bot: Ayaka):
        self = cls()
...
class Mod(commands.Cog):
    ...
    @checks.is_mod()
    async def raid_on(self, ctx: GuildContext, *, channel: discord.TextChannel | None = None):
    ...
    @checks.is_mod()
    async def raid_strict(self, ctx: GuildContext, *, channel: discord.TextChannel | None = None):
    ...
    @commands.guild_only()
    @checks.has_permissions(ban_members=True)
    async def mentionspam(self, ctx: GuildContext, count: int | None = None):
